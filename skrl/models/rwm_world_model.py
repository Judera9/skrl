...
from skrl.models.networks import MLPBase, RNNBase, MLPStateHead, MLPAuxiliaryHead
from skrl.models import Model

from skrl.utils.spaces import (
    flatten_tensorized_space,
    sample_space,
)


class SystemDynamicsEnsemble(Model):
    def __init__(
        self,
        observation_space,
        action_space,
        device: str,
        state_dim: int,  # TODO: handle in runner
        action_dim: int,  # TODO: handle in runner
        extension_dim: int,
        contact_dim: int,
        termination_dim: int,
        **kwargs,
        ...
        if kwargs:
            ...
            if unknown_keys:
                print(
                    "SystemDynamicsEnsemble.__init__ got unexpected arguments, which will be checked below: "
                    + str(unknown_keys)
                )
        ...
        self.history_horizon = kwargs.get("history_horizon", 1)
        self.architecture_config = kwargs.get("architecture_config", None)
        self.freeze_auxiliary = kwargs.get("freeze_auxiliary", False)
    ...
    def init_state_dict(self, role: str, inputs: Mapping[str, torch.Tensor] = {}) -> None:
        ...
        """
        if not inputs:
            inputs = sample_space(self.observation_space, backend="native", device=self.device)
        # init parameters
    ...
    def get_config(self):
        ...
        return {
            ...
            'extension_dim': self.extension_dim,
            ...
            'model_class': 'SystemDynamicsEnsemble'
        }
    
    def save_with_config(self, filepath, state_normalizer=None, action_normalizer=None):
        ...
        # Save normalizers if provided
        if state_normalizer is not None:
            checkpoint['state_normalizer'] = state_normalizer
        if action_normalizer is not None:
            checkpoint['action_normalizer'] = action_normalizer
            
        torch.save(checkpoint, filepath)
        print(f"Model and config saved to {filepath}")
        if state_normalizer is not None:
            print(f"State normalizer included in checkpoint")
        if action_normalizer is not None:
            print(f"Action normalizer included in checkpoint")
    
    @classmethod
    def from_checkpoint(cls, filepath, device="cpu"):
        """Load model from checkpoint with configuration"""
        checkpoint = torch.load(filepath, map_location=device, weights_only=False)
        config = checkpoint['config']
        
        # Get spaces from checkpoint
        ...
        model_params = {
            ...
            'extension_dim': config['extension_dim'],
            'contact_dim': config['contact_dim'],
            'termination_dim': config['termination_dim'],
            'ensemble_size': config['ensemble_size'],
            'history_horizon': config['history_horizon'],
            'architecture_config': config['architecture_config'],
            'freeze_auxiliary': config['freeze_auxiliary']
        }
        
        # Create model instance
        model = cls(observation_space, action_space, device, **model_params)
        
        # Load weights
        model.load_state_dict(checkpoint['state_dict'])
        print(f"Model loaded from {filepath}")
        
        # Extract normalizers if they exist
        state_normalizer = checkpoint.get('state_normalizer', None)
        action_normalizer = checkpoint.get('action_normalizer', None)
        
        if state_normalizer is not None:
            print(f"State normalizer loaded from checkpoint")
        if action_normalizer is not None:
            print(f"Action normalizer loaded from checkpoint")
        
        return model, state_normalizer, action_normalizer

    def act(self, inputs, model_ids=None, role=""):
        return self.compute(inputs, model_ids, role)

    def compute(self, inputs, model_ids=None, role=""):
        x_state_batch = inputs['x_state_batch']
        x_action_batch = inputs['x_action_batch']
